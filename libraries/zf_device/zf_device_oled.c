...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     OLED 格式化输出函数 (类似 printf)
// 参数说明     x               x 轴坐标设置 0-127
// 参数说明     y               y 轴坐标设置 0-7
// 参数说明     format          格式化字符串
// 参数说明     ...             可变参数
// 返回参数     void
// 使用示例     oled_printf(0, 0, "value=%d", 100);
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
void oled_printf (uint16 x, uint16 y, const char *format, ...)
{
  char oled_printf_buf[128];  // 格式化输出缓冲区
  va_list arg;                // 可变参数列表
  
  va_start(arg, format);      // 开始可变参数
  vsnprintf(oled_printf_buf, sizeof(oled_printf_buf), format, arg);
  va_end(arg);                // 结束可变参数
  
  oled_show_string(x, y, oled_printf_buf);
}
